...
public class RoomCheckInRequestKafkaListener implements KafkaConsumer<BookingRoomRequestAvro> {

    private final BookingCheckInRoomListener bookingCheckInRoomListener;
    private final RoomKafkaDataMapper roomKafkaDataMapper;



    @Override
    @KafkaListener(topics = "room-check-in-request", groupId = "room-checkin-request")
    public void receive(
        ...
        messages.forEach(message -> {
            try {
                ...
                bookingCheckInRoomListener.onCheckInRoom(domainMessage);
