 ...
 * về việc thanh toán tổng số tiền booking (total payment).
 * <p>
 * NGHIỆP VỤ:
 * - Nhận thông báo kết quả thanh toán tổng số tiền từ Payment Service
 ...
 * 5. Gửi đến PaymentResponseListener để xử lý business logic
 */
@Slf4j
@Component
...
public class PaymentResponseKafkaListener implements KafkaConsumer<BookingPaymentResponseAvro> {
    ...
    private void logBatchProcessingInfo(int messageCount,
                                        ...
                                        List<Integer> partitions,
                                        List<Long> offsets) {
        log.info("Nhận {} total payment response messages với keys: {}, partitions: {} và offsets: {}",
                messageCount,
                keys.toString(),
                partitions.toString(),
                offsets.toString());
    ...
    private void logPaymentProcessingStart(BookingPaymentResponseAvro bookingPaymentResponseAvro) {
        log.debug("Bắt đầu xử lý total payment response cho booking: {}, payment: {}, status: {}",
                bookingPaymentResponseAvro.getBookingId(),
    ...
    private void logPaymentProcessingSuccess(BookingPaymentResponseAvro bookingPaymentResponseAvro) {
        log.debug("Xử lý total payment response thành công cho booking: {}, payment: {}",
                bookingPaymentResponseAvro.getBookingId(),
                bookingPaymentResponseAvro.getPaymentId());
    }
    ...
    private void handleGenericException(BookingPaymentResponseAvro bookingPaymentResponseAvro,
                                        Exception exception) {
        log.error("Unexpected error khi xử lý total payment response cho booking: {}. Lỗi: {}",
                bookingPaymentResponseAvro.getBookingId(),
                exception.getMessage(),
                exception);
