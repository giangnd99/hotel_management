...
@Slf4j
public class PaymentPublishScheduler implements OutboxScheduler {

    private final PaymentOutboxService paymentOutboxService;
    private final DepositPaymentRequestMessagePublisher publish;
    ...
    public void processOutboxMessage() {
        Optional<List<PaymentOutboxMessage>> outboxMessagesResponse =
                paymentOutboxService.getListByBookingIdAndStatus(
                        OutboxStatus.STARTED,
                        SagaStatus.PROCESSING);
        if (outboxMessagesResponse.isPresent() && outboxMessagesResponse.get().size() > 0) {
            List<PaymentOutboxMessage> outboxMessages = outboxMessagesResponse.get();
            log.info("Received {} OrderApprovalOutboxMessage with ids: {}, sending to message bus!",
                    outboxMessages.size(),
                    outboxMessages.stream().map(outboxMessage ->
                            outboxMessage.getId().toString()).collect(Collectors.joining(",")));
            outboxMessages.forEach(outboxMessage ->
                    publish.sendDepositRequest(outboxMessage, this::updateOutboxStatus));
